...
class doctor_appointment(osv.osv):
	...
	_columns = {
		'attentiont_pediatrics_id': fields.many2one('doctor.attentions.pediatrics', 'Attentiont', ondelete='restrict', readonly=True),
	}


	def create_attentiont_pediatrics(self, cr, uid, doctor_appointment, modelo_crear,  context={}):
		"""
		...
		attentiont_number = {
			'attentiont_pediatrics_id': attentiont_id,
		}
	...
	def generate_attentiont(self, cr, uid, ids, context={}):
		...
		if self.pool.get('doctor.doctor').modulo_instalado(cr, uid, 'doctor_pediatrics', context=context):
				...
				return {
					...
					'view_type': 'form',
					'view_mode': 'form',
					'res_model': 'doctor.attentions.pediatrics',
					'res_id': attentiont_id or False,
					'view_id': [view_id] or False,
					'type': 'ir.actions.act_window',
					'context' : context or None,
